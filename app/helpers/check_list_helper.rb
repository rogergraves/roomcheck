  ...
  def link_to_problem(check_item_id)
    ...
    if result
      if result.severity > 0
        link_to("Edit Problem", edit_result_path(:id => result.id), :remote => true, :id => "resultpopup_#{result.id}", :class => "btn btn-info")
      else
        link_to("Report Problem", edit_result_path(:id => result.id), :remote => true, :id => "resultpopup_#{result.id}", :class => "btn btn-success")
      end
    else
